...
#"/vol/tensusers/ihendrickx/Data/Ancient-greek/HDT"
#None
...
def output(infn, docnum, sentences):
    ...
    for si, sentence in enumerate(sentences):
        ...
        for ID, form, POS in tokens:
            ...
            soout.write(tokstr(offset, offset+len(form), POS, idnum, form)+"\n")
        ...
        for dep, head, rel in deps:
            ...
            soout.write(depstr(idmap[dep], idmap[head], rel, ridnum)+"\n")
            ridnum += 1
...
def main(argv):
    ...
    if len(argv) > 2 and argv[1] == "-o":
        ...
        print("Writing output to", output_directory, file=sys.stderr)
        filenames = argv[3:]
    ...
    if fail_count > 0:
        print("WARNING: error in processing",fail_count,"/",filenames,"files, output is incomplete!", file=sys.stderr)
